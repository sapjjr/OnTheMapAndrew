    ...
    @IBAction func actionLogin(sender: AnyObject) {
        ...
        if outletEmail.text!.isEmpty {
            ...
            dispatch_async(dispatch_get_main_queue(), {
                let alert = UIAlertController(title: "Alert", message: ("Username Empty - try again"), preferredStyle: UIAlertControllerStyle.Alert)
            ...
            dispatch_async(dispatch_get_main_queue(), {
                let alert = UIAlertController(title: "Alert", message: ("Passcode Empty - try again"), preferredStyle: UIAlertControllerStyle.Alert)
                alert.addAction(UIAlertAction(title: "Click", style: UIAlertActionStyle.Default, handler: nil))
                self.presentViewController(alert, animated: true, completion: nil)
            })
    ...
    let task = session.dataTaskWithRequest(request) { data, response, error in
        ...
        guard let statusCode = (response as? NSHTTPURLResponse)?.statusCode where statusCode >= 200 && statusCode <= 299 else {
            ...
            if let response = response as? NSHTTPURLResponse {
                ...
                //aparently needs to be in a differnt thread this is achieved by the below!
                dispatch_async(dispatch_get_main_queue(), {
                let alert = UIAlertController(title: "Alert", message: ("There is a problem with either your username or passcode ! Status code: \(response.statusCode)!"), preferredStyle: UIAlertControllerStyle.Alert)
                alert.addAction(UIAlertAction(title: "Click", style: UIAlertActionStyle.Default, handler: nil))
                self.presentViewController(alert, animated: true, completion: nil)
                })
            }
        ...
        do {
                //parsedResult = try NSString(data: data, encoding: NSUTF8StringEncoding)!
